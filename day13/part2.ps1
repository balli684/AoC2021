...
#foreach ($instruction in $instructions) {
    if($instruction[0] -eq "y") {
        ...
        if ([int]$instruction[1] -gt ($grid.Count - 1) / 2) {
            $gridnew = New-Object "PSobject[][]" ([int]$instruction[1] - 1),$grid[0].Count
            for($y=0;$y -lt [int]$instruction[1];$y++) {
                for($x=0;$x -lt $grid[0].Count;$x++) {
                    ...
                    if($grid[($grid.Count - 1) - $y][$x]) {
                        $gridnew[$y][$x] = $grid[($grid.Count - 1) - $y][$x]
                    }
                }
            }
            ...
            [int]$dif = [int]$instruction[1] - [math]::Round(($grid.Count - 1) / 2)
            $dif
            $gridnew = New-Object "PSobject[][]" (($grid.Count - 1) - [int]$instruction[1]),$grid[0].Count
            for($y=0;$y -lt [int]$instruction[1];$y++) {
                for($x=0;$x -lt $grid[0].Count;$x++) {
                    if($grid[$y][$x]) {
                        $gridnew[$y+$dif][$x] = $grid[$y][$x]
                    }
                    if($grid[($grid.Count - 1) - $y][$x]) {
                        $gridnew[$y][$x] = $grid[($grid.Count - 1) - $y][$x]
                    }
